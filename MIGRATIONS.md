...
- **`drizzle/0011_academic_record.sql` (RS-BE-22/24/25 — copia del récord académico) — ⏳ PENDIENTE, acción del dueño.** Aditiva e idempotente (`CREATE TABLE IF NOT EXISTS`, sin tocar ninguna columna existente): crea `student_record_entry` (una fila por curso del récord: ciclo, código, vez, nota), `student_academic_snapshot` (una fila por alumno: PPA, ubicación relativa, créditos) y `student_period_summary` (una fila por ciclo del bloque "por período" de `layout.jsp`), las tres colgando de `student` con `ON DELETE CASCADE`. Diseño en `specs/features/academic-record/academic-record.spec.md` (RS-BE-19 a RS-BE-29).
  Aplicar con `bun run db:apply drizzle/0011_academic_record.sql`, no con `db:migrate`: el journal (`drizzle/meta/_journal.json`) se quedó en la `0009`, igual que con la `0010` de abajo. Con backup previo, siguiendo el protocolo de este documento.
