...
- **`drizzle/0013_chatbot_message_history.sql` (BR-CB-20, índice del historial del chatbot). ⏳ PENDIENTE, acción del dueño antes del merge de `fix/chatbot-delegados-bloques`.** Aditiva e idempotente, crea un solo índice, `idx_chatbot_message_session_created` sobre `chatbot_message (session_id, created_at)`, con `CREATE INDEX IF NOT EXISTS`, y no toca ninguna tabla, columna ni fila. `getRecentMessages` lo usa para leer los 10 últimos mensajes de la sesión sin ordenarla entera. `idx_chatbot_message_session` queda redundante y no se borra en esta migración. Diseño en `specs/features/chatbot/chatbot.spec.md` («Base de Datos» y BR-CB-20), con su aprobación de BD aparte de la de la spec.
  ...
  Antes de subir la rama al remoto o de abrir el PR, el dueño comprueba en Vercel que ningún despliegue de vista previa (entorno *Preview*) use el `DATABASE_URL` de producción, o desactiva esos despliegues para esta rama. `vercel.json` no los desactiva, y con la integración de Git de Vercel activa cada push de la rama arma uno por defecto. Si ese despliegue lee la base de producción, su primera petición al chatbot corre `purgeSessionsBeforeActivePeriod` (BR-CB-22) contra la base real, antes del conteo, del respaldo y de la aprobación de los pasos de abajo. Es el paso 1 de «Primera purga en producción», y sigue sin verificarse a la fecha de esta entrada.
  ...
  Al aplicar y desplegar se completa aquí la comprobación de los despliegues de vista previa, la fecha de la `0013`, el nombre de su backup y su verificación, el conteo y el respaldo de la primera purga y del borrado de BR-CB-22b, y la fecha del despliegue, sin copiar el contenido de ninguna conversación.
