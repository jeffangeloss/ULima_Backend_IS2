...
import { deriveWeeksFromPeriodDates } from "./schedule.repository.js";
...
export class ScheduleService {
  ...
  /**
   * Semanas académicas del período activo, con fallback en cascada para no
   * romper cuando faltan datos (ver `specs/features/schedule/schedule.spec.md`):
   *   1. filas de `academic_week` para el período activo;
   *   2. si no hay, se derivan de las fechas propias del período (`start_date`/`end_date`);
   *   3. si no hay período activo, lista vacía — cada método consumidor ya
   *      degrada a "sin info de semana" en vez de lanzar.
   * Se llama una sola vez por request y se pasa el resultado a quien lo necesite,
   * en vez de leerlo de nuevo por cada método (antes eran 16 semanas fijas en
   * memoria; ahora es lectura a BD).
   */
  private async resolveAcademicWeeks(): Promise<RawWeekRow[]> {
    const weeks = await this.repository.findAcademicWeeksForActivePeriod();
    if (weeks.length > 0) return weeks;

    const periodDates = await this.repository.findActivePeriodDates();
    if (!periodDates) return [];

    return deriveWeeksFromPeriodDates(periodDates.start_date, periodDates.end_date);
  }

  /** Cuerpo compartido de `getAssessments`/`getTeacherAssessments`: mapea filas crudas + semanas ya resueltas a la respuesta pública. */
  private buildAssessmentsResult(rawAssessments: RawAssessmentRow[], weeks: RawWeekRow[]): AssessmentsResult {
  ...
  async getSessions(studentId: number): Promise<SessionsResponse> {
    const [rows, weeks] = await Promise.all([
      this.repository.findActiveEnrollmentsWithSessions(studentId),
      this.resolveAcademicWeeks(),
    ]);
  ...
  async getAssessments(studentId: number): Promise<AssessmentsResult> {
    const [rawAssessments, weeks] = await Promise.all([
      this.repository.findActiveSyllabiAndAssessments(studentId),
      this.resolveAcademicWeeks(),
    ]);
    return this.buildAssessmentsResult(rawAssessments, weeks);
  }
  ...
  async getWeeklyLoad(studentId: number): Promise<WeeklyLoadResponse> {
    ...
    const assessmentsResult = this.buildAssessmentsResult(rawAssessments, weeks);
  ...
  async getTeacherSessions(teacherId: number) {
    const [classRows, advisingRows, weeks] = await Promise.all([
      this.repository.findTeacherSessionsWithClasses(teacherId),
      this.repository.findTeacherAdvisingSessions(teacherId),
      this.resolveAcademicWeeks(),
    ]);
  ...
  async getTeacherAssessments(teacherId: number): Promise<AssessmentsResult> {
    const [rawAssessments, weeks] = await Promise.all([
      this.repository.findTeacherSectionsAssessments(teacherId),
      this.resolveAcademicWeeks(),
    ]);
    return this.buildAssessmentsResult(rawAssessments, weeks);
  }
