...
- **Revisar el valor de `PORTAL_TIMEOUT_MS` (8 s) bajo su semántica nueva**: hasta este lote el timeout cubría solo hasta las cabeceras; ahora cubre la respuesta completa, incluida la lectura del cuerpo (necesario: un host que responde y deja de emitir bytes colgaba la importación entera). El default de 8 s se eligió con la semántica vieja y no se ha vuelto a calibrar. `layout.jsp` real pesa ~108 KB: con WebSphere frío, cabeceras lentas más cuerpo goteando podrían agotar el presupuesto donde antes no. Medir en la prueba manual antes de subirlo a ciegas.
