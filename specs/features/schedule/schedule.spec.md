...
- *Propuesta del 2026-09-25, pendiente de aprobación (`specs/features/recarga-portal/recarga-portal.spec.md`, RS-BE-58).* Cada elemento de `secciones` suma `asistenciaLeidaEn`, la hora de `enrollment.portal_attendance_read_at` en ISO 8601 UTC, o `null` si no hay ninguna lectura. Es un campo más y ninguno de los de antes cambia. Las filas del docente y de asesoría lo emiten siempre `null`.
