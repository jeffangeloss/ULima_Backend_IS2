...
>
> **Enmienda propuesta el 2026-09-25 por `specs/features/recarga-portal/recarga-portal.spec.md` y aprobada por el dueño el 2026-09-26 con esa spec, pendiente de implementar.** La ULima cambia el menú lateral del Aula Virtual, `parseAulas` ya no encuentra ninguna aula y la importación deja de actualizar la asistencia de todos. RS-BE-48 lee el menú en sus dos formatos. RS-BE-51 reutiliza este parser en `POST /portal-sync/refresh` y le suma el parámetro opcional `cicloEsperado`, que la importación también pasa, la marca `otroCiclo`, la marca `identityMismatch` solo para un código de alumno presente y distinto, y la identificación verificada `identificado`, que sale también cuando la página falla después en los totales. Con el menú de lista, el aviso de un aula cuya página falla nombra el aula y no el curso. RS-BE-58 guarda la hora de cada lectura en `enrollment.portal_attendance_read_at`, también desde la importación, y una fila que salta la guarda de lectura más reciente cuenta como actualizada (RS-BE-55). Hasta que la implementación llegue a producción, el backend desplegado sigue el texto sin enmendar.
...
> **Formato nuevo del menú (2026-09-25, RS-BE-48 de `recarga-portal.spec.md`, aprobado por el dueño el 2026-09-26 y pendiente de implementar).** El menú vivo ya no trae los arreglos `aNuAula`, `aCurs` ni `aSecc`. Por cada curso emite un `<li class="curso">` con la carrera, el nombre truncado y la sección, seguido del enlace `OpenAsistenciaAlumno('<aula>')`, y no trae el código del curso. La importación ya identifica cada curso por los ocultos `prm_sCoCurs` y `prm_sCoSecc` de la página de detalle, así que el menú solo aporta el aula y una sección para contrastar. Un aula cuya página falla, en cambio, queda sin curso conocido, y su aviso nombra el aula.
