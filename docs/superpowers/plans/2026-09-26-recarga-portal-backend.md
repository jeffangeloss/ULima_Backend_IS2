...
3. Aplica la `0015` con respaldo y su permiso, y completa su entrada en `MIGRATIONS.md`. Nadie despliega la rama antes, ni en *Preview*, así que el push de la rama y el merge esperan esa anotación, porque `main` se despliega en producción, *Preview* comparte su `DATABASE_URL` y sin la columna `POST /portal-sync/import` falla entera (RS-BE-58).
