...
| 2 | Academic Profile | `specs/features/academic-profile/academic-profile.spec.md` | US05, BR-AP-07, BR-AP-08 | R12, R13 | `src/modules/academic-profile` | Implementado. **Enmienda propuesta el 2026-09-25 por el test de especialidad y pendiente de aprobación**, con solo las especialidades oficiales (`is_active = true`) en el listado y en la elección (BR-AP-07) y el reemplazo de especialidades en una sola transacción (BR-AP-08) |
...
| 19 | Test de especialidad | `specs/features/specialty-test/specialty-test.spec.md` | RS-BE-37 … RS-BE-47 | Test que conduce Ulises y recomienda uno de los cuatro diplomas oficiales, con contenido versionado en el backend, puntaje transparente con aritmética exacta, hasta dos desempates, motivo redactado por Cohere con respaldo de plantillas y el último resultado del alumno para el Perfil. Enmienda Academic Profile (BR-AP-07 y BR-AP-08) | `src/modules/specialty-test`, `src/modules/index.ts`, `src/modules/academic-profile`, `src/shared/middleware/rate-limit.ts`, `src/db/schema/schema.ts`, `drizzle/0014_specialty_test_result.sql` y, sujetos a su decisión abierta 15, `scripts/specialty-test/generar.py` y `docs/specialty-test/` | **Propuesta del 2026-09-25, pendiente de la aprobación del dueño** y del cambio de BD de la `0014` (tabla `student_specialty_test_result`; la `0013` es la del historial del chatbot, ya en `main` y aplicada el 2026-09-25) |
