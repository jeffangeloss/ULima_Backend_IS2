...
[![ORM](https://img.shields.io/badge/ORM-Drizzle_·_34_tablas-C5F74F?style=for-the-badge&logo=drizzle&logoColor=black)](#-el-modelo-de-datos)

[![Superficie](https://img.shields.io/badge/Superficie-15_módulos_·_70_endpoints-1F3A5F?style=for-the-badge&logo=fastapi&logoColor=white)](#-la-api)
[![Verificación](https://img.shields.io/badge/Verificación-74_suites_·_14_473_líneas-6D28D9?style=for-the-badge&logo=testinglibrary&logoColor=white)](#-pruebas-y-calidad)
...
| **Superficie de API** | **15 módulos** · **70 endpoints**: **67 rutas** declaradas en los 16 archivos `*.routes.ts` más 3 de servicio en la raíz (`/`, `/health`, `/version`) · **7** no exigen token |
| **Modelo de datos** | **34 tablas** en `src/db/schema/schema.ts` · 9 archivos de migración |
| **Código** | 16 795 líneas TypeScript en 184 archivos bajo `src/` |
| **Verificación** | **74 suites** · 14 473 líneas de prueba · *mutation testing* con Stryker, una configuración por persona |
...
y 14 473 líneas de test**. Se despliega en un único sitio, Vercel, con preset `hono` y región fija
...
```mermaid
flowchart LR
  P["miUlima · webaloe<br/>HTML ISO-8859-1"] --> S["portal-sync<br/>10 parsers"]
...
| 8 | `registerModules(app)` | `server.ts:63` | Monta las 15 sub-apps, en el orden de [`src/modules/index.ts`](src/modules/index.ts):19-33. |
...
Registrados en [`src/modules/index.ts`](src/modules/index.ts):19-33, en este orden exacto. La columna **Archivos** es `nivel 1 / recursivo`; solo difiere en los dos módulos con subcarpetas.
...
| 15 | `portal-sync` | `/portal-sync` | 7 / 17 | Importación desde miUlima: descarga y parseo de matrícula, horario, récord, sílabos y delegados, con persistencia transaccional. | `authMiddleware` + `requireRole(...STUDENT_ROLES)`; `portalSyncRateLimit` en `/import` |

`STUDENT_ROLES` es la constante `["student", "delegate", "subdelegate"] as const` de `auth-middleware.ts:101`, expandida con spread en 10 módulos. El detalle endpoint por endpoint está en [La API](#-la-api); el modelo de permisos, en [Seguridad](#-seguridad).
...
├── test/                             # 74 suites `*.test.ts` · 14 473 líneas · 24 carpetas `HU##_<autor>`
...
único **parcial**; `IDX` = índice no único; `CK` = CHECK. Las 30 `id` enteras son
...
`drizzle/` tiene **9 archivos `.sql`** versionados y **6 snapshots** en `drizzle/meta/`, pero el
journal (`drizzle/meta/_journal.json`) tiene **7 entradas**. Ese desajuste es la historia completa.
...
`drizzle-kit generate` numera el archivo por **la cantidad de entradas del journal local** en el
momento de generarlo, y le pega un nombre-clave aleatorio (`flowery_jack_flag`, `slim_miracleman`,
`spicy_ironclad`, `groovy_kulan_gath`). Dos ramas que salen del **mismo estado del journal** producen
inevitablemente **el mismo ordinal**. Eso pasó tres veces en julio de 2026:
...
2. **Los dos huérfanos jamás se ejecutan** por `db:migrate`. Uno de ellos importa: una base
   reconstruida desde cero solo con `drizzle/` quedaría **sin `course_offering.total_hours`** y sin su
   CHECK, pese a que `schema.ts:268` y todos los snapshots ≥ 0001 la declaran.
3. **Una migración nueva con un `when` anterior al máximo registrado se salta en silencio.** No hay
   error; simplemente no corre.
4. La próxima `db:generate` producirá `0007_*.sql` + `0007_snapshot.json`, dejando permanentes el
   hueco `0005` en nombres de `.sql` y el hueco `0004` en snapshots.
...
> ⚠️ **Este módulo no comprueba que el docente dicte la sección.** Ni el controller ni el
> service llaman a `checkSectionOwnership` ni a nada equivalente, a diferencia de
> `schedule.service.ts:398-400` y `official-grades.service.ts:32-37`. Un docente cualquiera
> puede pedir el riesgo de inasistencia — con códigos y nombres de alumnos — de cualquier
> `sectionId`, y disparar el `notify` sobre ella.
...
|:---|:---|:---|:---|
| 1 | Calificar sección ajena | `403 NOT_SECTION_TEACHER` (líneas 346 y 350) | `403 NOT_SECTION_PROFESSOR`. **El string `NOT_SECTION_TEACHER` no existe en `src/`** |
...
| 13 | Body inválido en `PUT /academic-profile/me/specialties` | `400 INVALID_BODY` | `400 INVALID_REQUEST_BODY`. `INVALID_BODY` no existe en `src/` |
...
prueba.** Los 15 módulos tienen al menos un test que los toca, así que el hueco no es por módulo:
es por **capa**.
...
con ocho secciones —`db`, `auth`, `email`, `firebase`, `server`, `chatbot`, `portal`, `syllabus`—.
Las únicas excepciones son `src/server.ts` (para las variables que inyecta Vercel) y el tooling de
`src/db/`.

### Las 19 variables validadas
